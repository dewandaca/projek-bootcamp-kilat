    ...
    <div className="space-y-6">
      ...
      {data?.image_url && (
        <div className="flex justify-center mb-15">
          <img src={data.image_url} alt={data?.title} className="mt-4 rounded-md object-contain max-w-full w-full md:max-w-lg lg:max-w-xl" />
        </div>
      )}
      <p className="text-[#6D4C41] text-xl font-semibold" dangerouslySetInnerHTML={{ __html: content.replace(/\n/g, "<br />") }} />
      <p className="text-gray-500 text-xl">Posted on: {data?.created_at}</p>
